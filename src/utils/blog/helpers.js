...
export const formatPagination = (
  page = 1,
  limit = blogConfig.pagination.defaultLimit
) => {
  ...
  const validLimit =
    parsedLimit > 0 && parsedLimit <= blogConfig.pagination.maxLimit
      ? parsedLimit
...
export const sanitizeTagName = (tagName) => {
  return tagName.trim().replace(/^#/, '').replace(/\s+/g, ' ')
}
...
export const isValidTagName = (tagName) => {
  ...
  return (
    cleaned.length >= blogConfig.tag.nameMinLength &&
